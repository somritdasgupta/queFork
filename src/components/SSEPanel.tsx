...
export const SSEPanel = forwardRef<SSEPanelHandle, Props>(({ url }, ref) => {
    ...
    try {
      ...
      source.onmessage = (e) => {
        ...
        if (pausedRef.current) {
          ...
          setEvents((prev) => [...prev, evt, ...bufferRef.current.splice(0)]);
        }
      };
      source.onerror = () => {
        setConnected(false);
  ...
  }, [url, autoReconnect, reconnectCount, maxReconnects]);
      ...
      <div className="flex items-center gap-3 px-3 py-1 border-b border-border bg-surface-sunken shrink-0">
        ...
        {reconnectCount > 0 && (
          <span className="text-[9px] text-method-put font-bold">
            <RotateCcw className="h-2.5 w-2.5 inline mr-0.5" />×{reconnectCount}
          </span>
        )}
