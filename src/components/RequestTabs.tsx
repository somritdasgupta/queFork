...
const SCRIPT_SNIPPETS = {
  "pre-script": [
    {
      label: "Set timestamp",
      code: `qf.environment.set('timestamp', Date.now().toString());`,
    },
    {
      label: "Log request",
      code: `qf.log('Sending:', qf.request.method, qf.request.url);`,
    },
    {
      label: "Generate UUID",
      code: `qf.environment.set('uuid', crypto.randomUUID());`,
    },
    {
      label: "Set auth token",
      code: `const token = qf.environment.get('token');\nif (!token) qf.log('Warning: No token set');`,
    },
  ],
  "post-script": [
    {
      label: "Save token",
      code: `const json = qf.response.json();\nqf.environment.set('token', json.access_token || json.token);`,
    },
    {
      label: "Log status",
      code: `qf.log('Status:', qf.response.status, qf.response.statusText);`,
    },
    {
      label: "Chain request",
      code: `const data = qf.response.json();\nconst res = await qf.sendRequest({ url: 'https://api.example.com/next', method: 'GET' });\nqf.log('Chained:', res.status);`,
    },
    {
      label: "Save response ID",
  ...
  tests: [
    ...
    {
      label: "Has property",
      code: `test('Has data property', () => {\n  const json = qf.response.json();\n  expect(json).toHaveProperty('data');\n});`,
    },
    {
      label: "Array length",
      code: `test('Returns array', () => {\n  const json = qf.response.json();\n  expect(Array.isArray(json)).toBe(true);\n});`,
    },
    {
      label: "Not empty",
...
function runScriptSandbox(
  code: string,
  tab: "pre-script" | "post-script" | "tests",
): ScriptResult {
  if (!code.trim())
    return {
      status: "success",
    ...
    const qf = {
      ...
      environment: {
        ...
        set: function (key: string, val: string) {
          mockEnv[key] = val;
          logs.push(`✓ Set env.${key} = "${val}"`);
          return this;
        },
        remove: function (key: string) {
          delete mockEnv[key];
          logs.push(`✓ Removed env.${key}`);
          return this;
        },
        list: () =>
          Object.entries(mockEnv).map(([key, value]) => ({ key, value })),
        clear: () => {
          Object.keys(mockEnv).forEach((k) => delete mockEnv[k]);
          logs.push("✓ Cleared all env vars");
        },
      },
      variables: {
        ...
        list: () => ({ ...mockVars }),
        clear: function () {
          Object.keys(mockVars).forEach((k) => delete mockVars[k]);
          logs.push("✓ Cleared all vars");
          return this;
        },
      },
      request: {
      ...
      sendRequest: async (opts: any) => {
        logs.push(
          `→ sendRequest ${opts?.method || "GET"} ${opts?.url || "(no url)"}`,
        );
        return { status: 200, body: "{}", json: () => ({}), headers: {} };
      },
      response: {
        status: 200,
        statusText: "OK",
    ...
    if (testResults.length > 0) {
      ...
      testResults.forEach((t) => {
        if (t.passed) output.push(`  ✓ ${t.name}`);
          ...
          output.push(`  ✗ ${t.name}: ${t.error}`);
          errors.push(`Test failed: ${t.name}`);
        }
      });
    }
...
export function RequestTabs({ request, onChange }: Props) {
  ...
  const badge = (key: TabKey) => {
    if (key === "params")
      return request.params.filter((p) => p.enabled && p.key).length;
    if (key === "headers")
      return request.headers.filter((h) => h.enabled && h.key).length;
    return 0;
  };
  ...
  const handleRunScript = () => {
    ...
    setTimeout(() => {
      ...
      if (result.status === "success")
        toast.success(`Script ran in ${result.duration}ms`);
      else toast.error(`Script failed: ${result.errors[0]}`);
    }, 100);
  };

  const currentSnippets =
    SCRIPT_SNIPPETS[activeTab as keyof typeof SCRIPT_SNIPPETS] || [];
  const isScriptTab =
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {count > 0 && (
                <span className="ml-1 px-1 py-0.5 text-[9px] font-extrabold bg-primary/10 text-primary">
                  {count}
                </span>
              )}
            </button>
          ...
          <div className="flex flex-col flex-1 min-h-0">
              ...
              {bodyTypes.map((bt) => (
                <button
                  ...
                  onClick={() =>
                    onChange({
                      ...request,
                      body: { ...request.body, type: bt.value },
                    })
                  }
            ...
            {(request.body.type === "form-data" ||
              ...
              <KeyValueEditor
                ...
                onChange={(formData) =>
                  onChange({ ...request, body: { ...request.body, formData } })
                }
              />
            )}
                ...
                <CodeEditor
                  ...
                  onChange={(query) =>
                    onChange({
                      ...
                      body: {
                        ...request.body,
                        graphql: { ...request.body.graphql, query },
                      },
                    })
                  }
                  placeholder="query {\n  users {\n    id\n    name\n  }\n}"
                  ...
                  <CodeEditor
                    ...
                    onChange={(variables) =>
                      onChange({
                        ...
                        body: {
                          ...request.body,
                          graphql: { ...request.body.graphql, variables },
                        },
                      })
                    }
            ...
            {request.body.type === "none" && (
              <p className="text-[10px] text-muted-foreground/40 font-bold py-6 text-center">
        ...
        {isScriptTab && (
          <div className="flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between px-3 py-1 bg-surface-elevated border-b border-border shrink-0">
              <p className="text-[9px] text-muted-foreground/40 font-bold">
                {activeTab === "pre-script" && (
                  <>
                    Runs before request ·{" "}
                    <code className="font-mono bg-accent/50 px-0.5">
                      qf.environment.get(key)
                    </code>
                  </>
                )}
                {activeTab === "post-script" && (
              ...
              <div className="flex items-center gap-0.5">
                ...
                <button
                  ...
                  disabled={
                    scriptResult.status === "running" || !getScriptCode().trim()
                  }
            ...
            <CodeEditor
              ...
              onChange={(val) => {
                if (activeTab === "pre-script")
                  onChange({ ...request, preScript: val });
                else if (activeTab === "post-script")
                  onChange({ ...request, postScript: val });
              ...
              <div className="border-t border-border bg-surface-sunken shrink-0 max-h-[200px] overflow-auto">
                <div className="flex items-center gap-2 px-3 py-1 border-b border-border bg-surface-elevated">
                  ...
                  <span className="text-[9px] font-bold text-muted-foreground">
                    ...
                    {scriptResult.status === "success" &&
                      `Passed${scriptResult.duration !== undefined ? ` · ${scriptResult.duration}ms` : ""}`}
                    {scriptResult.status === "error" &&
                      `Failed${scriptResult.duration !== undefined ? ` · ${scriptResult.duration}ms` : ""}`}
                  </span>
                  <button
                    onClick={() =>
                      setScriptResult({
                ...
                <div className="px-3 py-1.5 font-mono text-[10px] leading-[1.6] space-y-0.5">
                  {scriptResult.output.map((line, i) => (
                    <div
                      key={`o-${i}`}
                      className={`${line.startsWith("  ✓") ? "text-status-success" : line.startsWith("  ✗") ? "text-destructive" : line.startsWith("✓") ? "text-status-success" : "text-foreground"}`}
                    >
                      {line}
                    </div>
                  ))}
