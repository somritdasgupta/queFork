...
import { Key, User, Lock, Globe, ChevronDown, RefreshCw } from "lucide-react";
      ...
      <div className="grid grid-cols-2 border-b border-border">
        <div className="relative flex items-center border-r border-border h-8">
          ...
          >
            ...
            <ChevronDown
              className={`h-3 w-3 text-muted-foreground/40 transition-transform ${open ? "rotate-180" : ""}`}
            />
          </button>
          {open && (
            <>
...
export function AuthEditor({ auth, onChange, onFetchToken }: Props) {
  const inputClass =
    "w-full h-8 px-3 text-[11px] font-mono bg-transparent focus:outline-none focus:bg-accent/30 placeholder:text-muted-foreground/20 text-foreground transition-colors";
  const labelClass =
    "text-[9px] font-bold uppercase tracking-wider text-muted-foreground/40 px-3 py-1.5";
  ...
  return (
    <div className="flex flex-col">
      ...
      {auth.type === "basic" && (
        <div className="flex flex-col">
          <div className="grid grid-cols-2 border-b border-border bg-surface-sunken">
            <div className={`${labelClass} border-r border-border`}>
              Username
            </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="grid grid-cols-[1fr_1fr_80px] border-b border-border">
            ...
            <div className="flex items-center justify-center">
              <select
                title="Add API key to"
                value={auth.apiKey?.addTo || "header"}
                onChange={(e) =>
                  onChange({
                    ...auth,
                    apiKey: { ...auth.apiKey!, addTo: e.target.value as any },
                  })
                }
      ...
      {auth.type === "oauth2" && (
        <div className="flex flex-col">
          <GrantTypeDropdown
            ...
            onScopeChange={(v) =>
              onChange({ ...auth, oauth2: { ...auth.oauth2!, scope: v } })
            }
            ...
            <>
              ...
              <div className="flex items-center border-b border-border">
                <input
                  ...
                  readOnly
                  title="Access token"
                  className={`${inputClass} opacity-40`}
                />
              </div>
              {auth.oauth2.tokenExpiresAt && (
                <div className="flex items-center gap-1.5 px-3 py-1 border-b border-border text-[9px]">
                  <span
                    className={`font-bold ${auth.oauth2.tokenExpiresAt > Date.now() ? "text-status-success" : "text-destructive"}`}
                  >
                    {auth.oauth2.tokenExpiresAt > Date.now()
                      ? `Expires in ${Math.round((auth.oauth2.tokenExpiresAt - Date.now()) / 1000)}s`
                      : "Token expired"}
                  </span>
                </div>
              )}
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-b border-border">
            ...
            <button
              ...
              aria-label={`Auto-refresh ${auth.oauth2?.autoRefresh ? "enabled" : "disabled"}`}
              onClick={() =>
                onChange({
                  ...auth,
                  oauth2: {
                    ...auth.oauth2!,
                    autoRefresh: !auth.oauth2?.autoRefresh,
                  },
                })
              }
              className={`w-7 h-4 rounded-full transition-colors flex items-center ${
                auth.oauth2?.autoRefresh
                  ? "bg-primary justify-end"
                  : "bg-muted-foreground/20 justify-start"
              }`}
            >
              <span className="w-3 h-3 mx-0.5 rounded-full bg-white shadow-sm" />
            </button>
          </div>
