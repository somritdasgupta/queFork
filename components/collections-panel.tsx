...
export function CollectionsPanel({
  ...
  const renderRequest = (collectionId: string, request: SavedRequest) => (
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            if (/^[0-9.]*$/.test(value) && (value.match(/\./g) || []).length <= 3) {
              // Split by dots and validate each group is max 999
              const groups = value.split('.');
              const isValid = groups.every(group => !group || (parseInt(group) <= 999 && group.length <= 3));
              if (isValid) {
          setNewCollection((prev) => ({
            ...prev,
            apiVersion: value,
        ...
        <Button 
          ...
          disabled={!newCollection.name.trim() || !newCollection.apiVersion.trim()}
        >
          Create Collection
        </Button>
        <Button
