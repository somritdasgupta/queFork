...
function logRequest(entry) {
  interceptedRequests.unshift({ ...entry, id: crypto.randomUUID(), timestamp: Date.now() });
  if (interceptedRequests.length > MAX_LOG_SIZE) interceptedRequests.length = MAX_LOG_SIZE;
  // Notify side panel
  chrome.runtime.sendMessage({ type: 'REQUEST_LOG_UPDATE', data: interceptedRequests }).catch(() => {});
}
...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    try {
      ...
      if (url.hostname === 'localhost' || url.hostname === '127.0.0.1' || url.hostname === '0.0.0.0' || url.hostname.endsWith('.local')) {
  ...
  { urls: ['http://localhost/*', 'http://127.0.0.1/*', 'http://0.0.0.0/*', 'https://localhost/*', 'https://127.0.0.1/*'] }
);

chrome.webRequest.onCompleted.addListener(
  (details) => {
    const entry = interceptedRequests.find(r => r.url === details.url && r.status === 'pending');
    if (entry) {
    ...
    if (enabled && proxyUrl) {
      try {
        ...
        chrome.proxy.settings.set({
          ...
          sendResponse({ success: true, message: `Proxy set to ${proxyUrl}` });
        });
      } catch (e) {
