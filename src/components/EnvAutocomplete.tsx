...
export function EnvAutocompleteInput({
  value,
  onChange,
  variables,
  placeholder,
  ...
  const findVariableContext = useCallback(
    (
      text: string,
      cursor: number,
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (ctx && variables.length > 0) {
      const filtered = variables.filter((v) =>
        v.key.toLowerCase().startsWith(ctx.prefix.toLowerCase()),
      );
    ...
    if (showSuggestions) {
      ...
      if (e.key === "Enter" || e.key === "Tab") {
        if (suggestions[selectedIdx]) {
          e.preventDefault();
          applySuggestion(suggestions[selectedIdx].key);
          return;
        }
      }
      if (e.key === "Escape") {
  ...
  return (
    <div className="relative flex-1 min-w-0">
      ...
      {showSuggestions && (
        <div
          ref={dropdownRef}
          className="absolute top-full left-0 right-0 z-[60] mt-0.5 bg-card border border-border shadow-lg max-h-[160px] overflow-y-auto"
        >
          {suggestions.map((v, i) => (
            <button
              key={v.key}
            ...
            >
              ...
              <span className="text-[9px] text-muted-foreground font-mono truncate max-w-[100px] ml-2">
                {v.value}
              </span>
            </button>
          ))}
