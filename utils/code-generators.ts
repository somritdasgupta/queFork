import {
  SiCurl, SiClojure, SiSharp, SiGo, 
  SiJavascript, SiPython, SiPhp, SiRuby, 
  SiSwift, SiRust, SiKotlin, 
  SiPowers, SiPerl,
  SiAxios
} from 'react-icons/si';
import { VscJson } from 'react-icons/vsc';
import type { IconType } from 'react-icons';
import { formatCodeWithLanguage } from './code-formatters';
...
export const languageConfigs: Record<string, LanguageConfig> = {
  curl: {
    ...
    generator: ({ url, method, headers = {}, body }) => {
      ...
      const code = `
        ...
        ${headerStrings} ${body ? `\\\n  -d '${body}'` : ''}
      `;
      return formatCodeWithLanguage(code, 'bash');
    }
  },
  python: {
    name: 'Python',
    ...
    generator: ({ url, method, headers = {}, body }) => {
      const code = `
        ...
        try:
            ...
            print(response.text)
      `;
      return formatCodeWithLanguage(code, 'python');
    }
  },
  axios: {
    name: 'Axios',
    ...
    generator: ({ url, method, headers = {}, body }) => {
      const code = `
        ...
        makeRequest();
      `;
      return formatCodeWithLanguage(code, 'babel');
    }
  },
  fetch: {
    name: 'Fetch',
  ...
  kotlin: {
    ...
    generator: ({ url, method, headers = {}, body }) => `
      ...
      val request = Request.Builder()
          .url("${url}")
          .method("${method.toUpperCase()}", ${body ? `RequestBody.create(
  ...
  perl: {
    ...
    generator: ({ url, method, headers = {}, body }) => `
      ...
      ${Object.entries(headers).map(([k, v]) => `$req->header('${k}' => '${v}');`).join('\n')}
